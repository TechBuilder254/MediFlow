...
const schema = z.object({
  ...
  nationalId: z.string().min(5, 'National ID or Passport is required'),
      ...
      <div className="max-w-lg mx-auto">
        ...
        <div className="bg-white rounded-2xl border border-navy-100 p-8 shadow-sm">
          ...
          <form onSubmit={form.handleSubmit(onRegister)} className="mt-6 space-y-4">
            ...
            <Input label="National ID / Passport" {...form.register('nationalId')} error={form.formState.errors.nationalId?.message} placeholder="12345678" />
            <Input label="Phone Number" {...form.register('phone')} error={form.formState.errors.phone?.message} placeholder="0712 345 678" />
            <Input label="Email (Optional)" type="email" {...form.register('email')} error={form.formState.errors.email?.message} placeholder="you@email.com" />
